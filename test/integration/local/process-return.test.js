...
describe('process acknowledgement', () => {
  beforeAll(async () => {
    ...
    const blockBlobClient = inboundContainer.getBlockBlobClient('mock Return File.csv')
  ...
  test('archives file on success', async () => {
    ...
    expect(fileList.filter(x => x === 'mock Return File.csv').length).toBe(1)
  })

  test('ignores unrelated file', async () => {
    const blockBlobClient = inboundContainer.getBlockBlobClient('ignore me.csv')
    await blockBlobClient.uploadFile(TEST_FILE)
    await processing.start()
    const fileList = []
    ...
    expect(fileList.filter(x => x === 'ignore me.csv').length).toBe(1)
  })
